...
SELECT [04_Set_Based_Type_1SK]
      ...
      ,[IsRowDeleted]
      ,[LastUpdateDate]
  FROM [Target].[04_Set_Based_Type_1]
 ORDER BY [BigNumber]
...
EXECUTE [CUD].[04_Set_Based_Type_1] @MaxInsertRowsT1, @UpdateFromT1, @UpdateThruT1, @DeleteFrom, @DeleteThru, @IncludeType2ChangeT1
...
SELECT [04_Set_Based_Type_1SK]
      ...
      ,[LastUpdateDate]
  FROM [Target].[04_Set_Based_Type_1]
 WHERE [BigNumber] > (SELECT MAX(BigNumber) -50 FROM [Target].[04_Set_Based_Type_1] WHERE TargetCIUD = 'Created' )
 ORDER BY [BigNumber]
...
SELECT [04_Set_Based_Type_1SK]
      ...
      ,[LastUpdateDate]
  FROM [Target].[04_Set_Based_Type_1] 
 WHERE [BigNumber] > (SELECT MIN(BigNumber) -3 FROM [Target].[04_Set_Based_Type_1] WHERE TargetCIUD = 'Update' )
   AND [BigNumber] < (SELECT MAX(BigNumber) +3 FROM [Target].[04_Set_Based_Type_1] WHERE TargetCIUD = 'Update' )
 ORDER BY [BigNumber]
