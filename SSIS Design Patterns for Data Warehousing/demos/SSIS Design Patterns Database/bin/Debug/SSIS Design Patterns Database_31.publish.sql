...
BEGIN
  ...
  SET @columnDefs = ''
	...
	SET @columnDefs = @columnDefs + '[SourceCIUD] NVARCHAR(20) NOT NULL, '
	SET @columnDefs = @columnDefs + '[TargetCIUD] NVARCHAR(20) NOT NULL, '
  ...
  ELSE
    BEGIN
	    SET @columnDefs = @columnDefs + '[EffectiveDate] DATETIME NOT NULL, '
...
BEGIN
  ...
  SET @sqlStatement = @sqlStatement + '  EXEC DML.DeleteFromSourceTable ''' + @TableSchemaName + ''', ''' + @TableName + ''', ' + CAST(@DeleteFrom AS NVARCHAR) + ', ' + CAST(@DeleteThru AS NVARCHAR)+ ', ' + CAST(@IncludeType2Change AS NVARCHAR) + ' ' + @crlf
  SET @sqlStatement = @sqlStatement + ' ' + @crlf
  SET @sqlStatement = @sqlStatement + 'END ' + @crlf
...
BEGIN
  ...
  EXEC [DDL].[CreateETLTable] '04_Set_Based_Type_2', @IncludeType2
